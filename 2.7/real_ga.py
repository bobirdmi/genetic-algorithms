...
class RealGA(StandardGA):
    ...
    def _get_mut_bit_offset(self):
        ...
        if self._bin_length == 32:
            ...
            raise ValueError(u'Wrong floating point binary length: may be only 32 or 64.')
    ...
    def _get_chromosome_return_value(self, chromosome):
        ...
        try:
            ...
            if length < 1:
                raise ValueError(u'The given chromosome is empty!')
            elif length > 1:
                return chromosome
            else:
                return chromosome[0]
        except TypeError:
            raise ValueError(u'The given chromosome is not a list!')
    ...
    def _replace_bits(self, source, target, start, stop):
        ...
        if start < 0 or start >= self._bin_length or \
            ...
            raise ValueError(u'Replacement interval error')
        ...
        child = []
        for source_ind, target_ind in izip(origin_source, origin_target):
